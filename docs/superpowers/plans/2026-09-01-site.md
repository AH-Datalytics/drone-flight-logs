...
- No portal bar, no gradients, no rounded corners, no emoji (house rule). Editorial, dense, data-first.
...
:root {
  ...
  --rule: #d9d9d9; --rule-2: #ececec; --paper: #ffffff; --paper-2: #f6f6f4;
  --accent: #1f4e79; --warn: #9a6700; --bad: #a12a2a; --muted-fill: #cfd8e3;
  --mono: ui-monospace, "SF Mono", Menlo, Consolas, monospace;
  --sans: -apple-system, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
}
* { box-sizing: border-box; }
html, body { margin: 0; padding: 0; background: var(--paper); color: var(--ink); font: 15px/1.45 var(--sans); }
a { color: var(--accent); text-decoration: none; } a:hover { text-decoration: underline; }
.wrap { max-width: 1180px; margin: 0 auto; padding: 0 20px; }
.masthead { border-bottom: 2px solid var(--ink); padding: 18px 0 12px; display: flex; align-items: baseline; justify-content: space-between; gap: 16px; }
.masthead h1 { margin: 0; font-size: 20px; letter-spacing: .01em; } .masthead h1 a { color: var(--ink); }
.masthead nav a { margin-left: 18px; color: var(--ink-2); font-size: 14px; }
.footer { border-top: 1px solid var(--rule); margin-top: 48px; padding: 16px 0 40px; color: var(--ink-3); font-size: 13px; }
h2 { font-size: 22px; margin: 28px 0 8px; } h3 { font-size: 15px; text-transform: uppercase; letter-spacing: .06em; color: var(--ink-2); margin: 24px 0 8px; }
p.lede { font-size: 17px; color: var(--ink-2); max-width: 760px; }
.num { font-variant-numeric: tabular-nums; text-align: right; }
table.data { width: 100%; border-collapse: collapse; font-size: 14px; }
table.data th, table.data td { padding: 6px 8px; border-bottom: 1px solid var(--rule-2); text-align: left; vertical-align: top; }
table.data th { border-bottom: 1px solid var(--ink); font-weight: 600; white-space: nowrap; cursor: pointer; user-select: none; }
table.data th.num, table.data td.num { text-align: right; }
table.data tbody tr:hover { background: var(--paper-2); }
.badge { font-size: 11px; text-transform: uppercase; letter-spacing: .06em; padding: 1px 6px; border: 1px solid var(--rule); color: var(--ink-2); white-space: nowrap; }
.badge.ok { border-color: #8fb996; color: #2f6b3a; } .badge.stale { border-color: #d9b45a; color: var(--warn); } .badge.unreachable, .badge.retired { border-color: #d29a9a; color: var(--bad); }
.stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 0; border-top: 1px solid var(--ink); border-bottom: 1px solid var(--rule); margin: 16px 0 8px; }
.stat { padding: 12px 12px 12px 0; border-right: 1px solid var(--rule-2); } .stat:last-child { border-right: 0; }
.stat .v { font-size: 26px; font-weight: 600; font-variant-numeric: tabular-nums; line-height: 1.1; } .stat .l { font-size: 12px; color: var(--ink-3); text-transform: uppercase; letter-spacing: .05em; margin-top: 4px; }
.controls { display: flex; gap: 12px; align-items: center; margin: 12px 0; flex-wrap: wrap; }
.controls input[type=search] { font: inherit; padding: 7px 10px; border: 1px solid var(--ink-3); min-width: 320px; }
.controls label { font-size: 14px; color: var(--ink-2); }
.btn { display: inline-block; padding: 8px 14px; border: 1px solid var(--ink); color: var(--ink); font-weight: 600; font-size: 14px; }
.btn:hover { background: var(--ink); color: var(--paper); text-decoration: none; }
.note { background: var(--paper-2); border-left: 3px solid var(--ink-3); padding: 10px 14px; font-size: 14px; color: var(--ink-2); margin: 12px 0; }
.charts { display: grid; grid-template-columns: repeat(auto-fit, minmax(340px, 1fr)); gap: 24px 32px; margin-top: 8px; }
.chart h3 { margin-top: 8px; }
.pager { display: flex; gap: 8px; align-items: center; font-size: 14px; margin: 10px 0; } .pager button { font: inherit; padding: 4px 10px; border: 1px solid var(--ink-3); background: var(--paper); cursor: pointer; } .pager button:disabled { color: var(--ink-3); cursor: default; }
.agency-head { display: flex; justify-content: space-between; align-items: flex-end; gap: 16px; flex-wrap: wrap; margin-top: 24px; }
.agency-head .meta { color: var(--ink-2); font-size: 14px; }
