...
describe('discoverSkydioDashboards', () => {
  it('lists dashboards in the Skydio org and resolves org uuids through the web map', async () => {
    ...
    expect(d).toEqual([
      { item_id: '81fc8f0745944ddfbf773850cf28eca8', title: 'Milwaukee Police Department', org_uuid: ORG, modified: '2025-08-27T12:26:40.000Z' },
      { item_id: '46c00955a6a34935879f71a87f5934e0', title: '[TEMPLATE] Skydio Transparency Dashboard', org_uuid: null, modified: '2025-08-27T12:26:40.000Z' },
    ]);
  });
});
...
export function validateDataDir(dataDir: string): string[] {
  ...
  for (const a of reg.agencies) {
    ...
    if (a.status === 'ok' || a.status === 'stale' || a.status === 'unreachable') {
...
jobs:
  pull:
    ...
    steps:
      ...
      - name: Pull
        run: |
          if [ -n "${{ inputs.only }}" ]; then npm run pull -- --only=${{ inputs.only }}; else npm run pull; fi
      - name: Validate
        run: npm run validate:data
