...
import type { SiteAgency } from '../pipeline/build.js';
...
const site: { agencies: SiteAgency[] } = JSON.parse(readFileSync(join(DATA, 'site', 'agencies.json'), 'utf8'));
let n = 0;
for (const a of site.agencies) {
  const src = join(DATA, 'site', 'flights', `${a.agency_id}.json`);
  ...
  const cols = ['agency', ...COLUMNS.filter(c => c !== 'extra'), 'source', ...extraKeys];
  const lines = [cols.join(','), ...recs.map(r => [a.display_name, ...COLUMNS.filter(c => c !== 'extra').map(c => (r as any)[c]), (r as any).source, ...extraKeys.map(k => r.extra?.[k])].map(q).join(','))];
